#!/usr/bin/env bash
# FP4 C4 indexer (sglang#37353) on DPA+TBO, c64, 3600 s, mem-frac 0.85.
# A/B on HSA_NO_SCRATCH_RECLAIM: this arm runs ==1. Its partner
# fp4_dptbo_c64_reclaim0.sh is identical apart from that variable and RESULT_DIR.
#
# mem-frac is 0.90, matching the c128 pair. Note this deliberately gives up
# comparability with the existing dptbo-c64 row, which ran at 0.85: the only
# reading this pair supports is reclaim-0-vs-1, not FP4-vs-baseline at c64.
# 0.90 also leaves less free VRAM, so the reclaim=1 arm is more exposed to the
# OOR abort that killed the c128 attempt -- which is itself part of the answer.
set -u
...
# The variable under test. The container default is 1, which retains the
# scratch a large-grid kernel allocated; at c128 that walked free VRAM to zero
# and aborted the arm at 44/60 min with HSA_STATUS_ERROR_OUT_OF_RESOURCES.
# c64 is less memory-pressured, so =1 is expected to survive here -- which is
# what makes a like-for-like throughput comparison possible at all.
...
export RESULT_DIR=/workspace/results/fp4-dptbo-c64-reclaim1
