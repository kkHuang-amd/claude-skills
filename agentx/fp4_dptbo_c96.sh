#!/usr/bin/env bash
# FP4 C4 indexer (sglang#37353) on DPA+TBO, c96, 3600 s.
# Byte-for-byte the settings of fp4_dptbo_c128.sh except CONC=96 (and the
# result paths), so it extends the FP4 curve at a third operating point.
# max-running-requests and cuda-graph-max-bs are derived from CONC by the
# launcher, so they follow automatically (192 / 96 here vs 256 / 128 at c128).
set -u
...
export CONC=96 DURATION=3600 PORT=8888
...
export RESULT_DIR=/workspace/results/fp4-dptbo-c96
export RESULT_FILENAME="dsv4_fp4_sglang_tp8-pp1-dcp1-pcp1-ep1-dpatrue_disagg-false_spec-mtp_agentic_c96"
