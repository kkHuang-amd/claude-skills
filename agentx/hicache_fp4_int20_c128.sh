...
RESULT_DIR=/workspace/results/hicache-fp4-int20-c128
mkdir -p "$RESULT_DIR"
...
echo "=== THE PAIR: PASS needs ITL p90 <= 61.5 ms AND TTFT avg <= 10.9 s ==="
python3 "$HERE/arm_report.py" hicache-fp4-int20-c128 interval20-c128
echo "--- ATOM c128: 30,709 tok/s/chip, ITL p90 61.5 ms, TTFT 10.9 s ---"
echo "--- also compare to dptbo-c128 (interval 10, no hicache, no FP4): ---"
python3 "$HERE/arm_report.py" hicache-fp4-int20-c128 dptbo-c128 2>&1 | tail -12
